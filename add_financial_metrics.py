...
def add_financial_metrics():
    try:
        ...
        for item in inventory_items:
            # Generate realistic purchase and selling prices
            purchase_price, selling_price = generate_realistic_prices(item['price'])
            profit, profit_margin, markup = calculate_profit_margins(purchase_price, selling_price)
            
            # Calculate inventory values
            inventory_value = purchase_price * item['quantity']
            potential_profit = profit * item['quantity']
        ...
        for sale in sales:
            ...
            item = next((i for i in inventory_items if i['_id'] == sale['item_id']), None)
            if item:
